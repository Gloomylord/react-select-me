...
export default class ReactSelectMe extends PureComponent {
    ...
    renderList = () => {
        ...
        let listContent;
        if (rowCount) {
                listContent = options.map((option,index )=> this.renderOption(option, selectedOptions,{},index));
        } else {
            listContent =
    ...
    renderOption = (option, selectedOptions, style,index) => {
        ...
        const className = cs( s.dd__option, {
            dd__selectedOption: isSelected,
            [s.dd__selectedOption]: isSelected,
            'dd__option': !(option.value===null),
            'group': option.group && option.value===null,
            'pl': !(option.value===null) && option.group
        });
        ...
        if(option.value === null){
            return (
                <div key={option.group} style={style}>
                    {label}
                </div>
            );
        } else {
            ...
            return (
                <div tabIndex='0'
                     key={this.getProp(option, valueKey)}
                     style={style}
                     className='tab'
                     onBlur={()=>{rowCount===(index+1) && this.onToggle()}}
                     onKeyDown={this.pressEnterSelectItem(option)}
    ...
    renderSelectedBlock = () => {
        ...
        return (
            <div className={selectedBlockClasses}>
                {selectedElements}
                {noOptionsSelected && (!searchable || !opened) &&
    ...
    renderSelectedItem = option => {
        ...
        return (
            <div className={selectedOptionClasses} key={this.getProp(option, valueKey)}>
                ...
                {multiple && (
                    <div className={crossIconClasses} onClick={(e)=> {
                        this.onRemoveSelected(option)(e);
                    }
                    }>
                        ×
                    </div>
     ...
     **************** Events ******************
     **************************************** */
    onChange = (option, removeFromSelectedBlock) => (e) => {
        if(e) {
            e.preventDefault();
        }
    ...
    onToggle = e => {
        const {props: {multiple,searchable, beforeOpen, beforeClose, isOpened, disabled}, state: {opened}} = this;
     ...
     **************************************** */
    render() {
        const {error, multiple, disabled,clearable, s} = this.props;
        const {opened} = this.state;
        ...
        return (
            <div className={wrapperClassnames}
                ...
                <div
                    ...
                    {clearable && <div className='valueClear' onClick={this.clearValue}>×</div>}
                    {this.renderIcon()}
                </div>
                {this.renderList()}
            </div>
